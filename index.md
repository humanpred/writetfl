...
Tables and other grid grobs (e.g. from `gt` or `gridExtra`) are also
accepted as `content`, so you can mix figures and tables in one PDF:
...
| `...`       |                     | Passed to [`export_tfl_page()`](https://humanpred.github.io/writetfl/reference/export_tfl_page.md); see below. |
