...
Set `tfl_colspec(wrap = TRUE)` or `tfl_colspec(wrap = FALSE)` to override
the table-level setting for one column. `wrap = NA` (the default) means
...
whitespace at the break. `wrap_breaks()` lets you opt in to break-after
characters that *stay* on the upper line:
...
`overflow_action = "warn"` on `export_tfl()` downgrades it to a warning
and produces output for diagnosis. See
`vignette("v04-troubleshooting")` for the full overflow story.
...
blanked cells like an HTML `<td rowspan="N">` instead of inflating only
...
| `wrap_breaks` | `wrap_breaks()` | Break-character spec — defaults to whitespace; opt into `keep_before` chars like `-` or `/` |
